...
function update () {Object.assign
                ...
                const currSP = cont.querySelector (".currSP"); const currAP =cont.querySelector (".currAP");
                ...
                currSP.textContent=jatekos.startSP; currAP.textContent=jatekos.currAP;
                ...
                mres.textContent=jatekos.Mell; sres.textContent=jatekos.Sell;
                 Attributemax();
                for (const key in attributespan) {attributespan[key].textContent=`${jatekos[key]} / ${alkaszt.limit[key] || 50}`}; 
...
const filteredRows = dataStore.Stats.filter (row =>Number(row.SP) === alkaszt.startSP); alkaszt.startAP=alkaszt.startSP; alkaszt.currAP=alkaszt.startAP; update();
...
alkaszt.Mell=Math.floor((alkaszt.Intelligencia+alkaszt.Tehetség)/2); alkaszt.Sell=Math.floor((alkaszt.Felismerés+alkaszt.Inspiráció)/2); update()}, 2000); 
